 ...
 *      			 Generates PWM Signal of time period 1 Sec and Duty Cycles of 25%, 45%, 75%
 *      			 and 90% on channel 1, 2, 3, 4 resp.
 *
 */
...
int main(void)
{
	...
	if(HAL_TIM_PWM_Start(&TIM2_Handler, TIM_CHANNEL_1))
	{
		Error_Handler();
	}

	if(HAL_TIM_PWM_Start(&TIM2_Handler, TIM_CHANNEL_2))
	{
		Error_Handler();
	}

	if(HAL_TIM_PWM_Start(&TIM2_Handler, TIM_CHANNEL_3))
	{
		Error_Handler();
	}

	if(HAL_TIM_PWM_Start(&TIM2_Handler, TIM_CHANNEL_4))
	{
		Error_Handler();
	}
...
void TIMER2_Init(void)
{
	...
	TIM2_Handler.Init.Period = 65274-1;
	TIM2_Handler.Init.Prescaler = 765;
	...
	//25% Duty
	TIM2_PWM_Init.Pulse = (TIM2_Handler.Init.Period * DUTY_25) / 100U;
	...
	//45% Duty
	TIM2_PWM_Init.Pulse = (TIM2_Handler.Init.Period * DUTY_45) / 100U;
	if(HAL_TIM_PWM_ConfigChannel(&TIM2_Handler, &TIM2_PWM_Init, TIM_CHANNEL_2) != HAL_OK)
	{
		Error_Handler();
	}

	//75% Duty
	TIM2_PWM_Init.Pulse = (TIM2_Handler.Init.Period * DUTY_75) / 100U;
	if(HAL_TIM_PWM_ConfigChannel(&TIM2_Handler, &TIM2_PWM_Init, TIM_CHANNEL_3) != HAL_OK)
	{
		Error_Handler();
	}

	//95% Duty
	TIM2_PWM_Init.Pulse = (TIM2_Handler.Init.Period * DUTY_95) / 100U;
	if(HAL_TIM_PWM_ConfigChannel(&TIM2_Handler, &TIM2_PWM_Init, TIM_CHANNEL_4) != HAL_OK)
	{
		Error_Handler();
	}
